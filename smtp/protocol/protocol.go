...
var (
	ErrBadlyFormedParameter = errors.New("badly formed parameter")
...
type Protocol struct {
	...
	MessageReceivedHandler func(*Message) (string, error)
...
func NewProtocol() *Protocol {
	p := &Protocol{
		Hostname:          "mailcage.example",
		Ident:             "ESMTP MailCage",
		State:             INVALID,
		MaximumLineLength: -1,
		MaximumRecipients: -1,
	}
...
func (proto *Protocol) Command(command *Command) (reply *Reply) {
	...
	case AUTHPLAIN == proto.State:
		...
		if proto.ValidateAuthenticationHandler != nil {
			...
			if len(bits) < 3 {
				return ReplyError(ErrBadlyFormedParameter)
			}
